...
export const createJobAction = async (data: JobFormData): Promise<{ status: 'success' | 'error'; message: string; }> => {
    try {
...
export const getJobByIdAction = async (jobId: number): Promise<{ status: 'success' | 'error'; message?: string; data?: any }> => {
    try {
        const currentUser = await getCurrentUser();
...
export const updateJobAction = async (jobId: number, values: any): Promise<{ status: 'success' | 'error'; message: string; }> => {
    try {
        const currentUser = await getCurrentUser();
        if (!currentUser || currentUser.role !== 'employer') return { status: "error", message: "Unauthorized." };
