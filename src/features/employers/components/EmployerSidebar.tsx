...
export const EmployerSidebar: React.FC = () => <div className="w-64 bg-card border-r border-border fixed bottom-0 top-0">
  <div className="p-6">
  ...
  <nav className="px-3 space-y-1">
    {navigationItems.map((curNav) => {
      ...
      return <Link key={curNav.name} href={curNav.href || '#'} className=" flex items-center gap-3 px-3 py-2 text-sm font-medium rounded-lg transition-colors">
        <Icon />
        {curNav.name}
      </Link>;
    })}
